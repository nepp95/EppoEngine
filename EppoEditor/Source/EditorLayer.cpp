...
namespace Eppo
{
    ...
    auto EditorLayer::OnAttach() -> void
    {
        ...
        m_PanelManager->GetPanel<ContentBrowserPanel>(CONTENT_BROWSER_PANEL)
            ->SetOpenSceneCallback(
                [this](const AssetHandle handle) -> void
                {
                    OpenScene(handle);
                }
            );
    ...
    auto EditorLayer::OnUIRender() -> void
    {
        ...
        if (io.ConfigFlags & ImGuiConfigFlags_DockingEnable)
        {
            ...
            // Reserve a bottom strip for the non-dockable status bar so docked panels
            // cannot overlap it; the bar is rendered into the freed space below.
            const float statusBarHeight = 24.0f;
            const ImVec2 avail = ImGui::GetContentRegionAvail();
            ImGui::DockSpace(dockspaceId, ImVec2(0.0f, std::max(0.0f, avail.y - statusBarHeight)), dockspaceFlags);
        }
        ...
        if (ImGui::BeginMenuBar())
        {
            ...
            ImGui::GetWindowDrawList()->AddLine(
                { dividerPos.x, dividerPos.y + 4.0f }, { dividerPos.x, dividerPos.y + dividerHeight - 4.0f },
        ...
        ImGui::Image(
            ImGuiEx::CreateTextureRef(finalImage->GetTexture()),
        ...
        // Status bar occupies the bottom strip reserved above; drawn here so the
        // DockSpace host window is still current.
        m_StatusBar.Render();
    ...
    auto EditorLayer::OnScenePlay() -> void
    {
        ...
        const UUID selectedUUID = m_PanelManager->GetSelectedEntity() ? m_PanelManager->GetSelectedEntity().GetUUID() : UUID{};
    ...
    auto EditorLayer::OpenProject() -> void
    {
        ...
        FileDialog::OpenFile(
            "OpenProject", "Open Project", FileDialog::BuildFilter("EppoEngine Project", { "epproj" }), Project::GetProjectsDirectory(),
    ...
    auto EditorLayer::ExportGame() -> void
    {
        ...
        FileDialog::OpenFolder(
            ...
            [this, project](const std::filesystem::path& parentDirectory)
            {
                ...
                try
                {
                    m_ExportFuture = std::async(
                        std::launch::async,
                        [project, options = std::move(options)]() mutable -> ProjectExportResult
                        {
    ...
    auto EditorLayer::OpenScene() -> void
    {
        ...
        FileDialog::OpenFile(
            "OpenScene", "Open Scene", FileDialog::BuildFilter("EppoEngine Scene", { "epscene" }), Project::GetAssetsDirectory(),
            [this](const std::filesystem::path& path)
            {
                OpenScene(path);
            }
    ...
    auto EditorLayer::SaveSceneAs() -> void
    {
        ...
        FileDialog::SaveFile(
            "SaveSceneAs", "Save Scene As", FileDialog::BuildFilter("EppoEngine Scene", { "epscene" }), Project::GetAssetsDirectory(),
            [this](const std::filesystem::path& path)
            {
    ...
    auto EditorLayer::UpdateImGuizmo() -> void
    {
        ...
        if (m_SelectedEntity && m_SelectedEntity.HasComponent<TransformComponent>())
        {
            ...
            glm::mat4 transform = glm::translate(glm::mat4(1.0f), tc.Translation) * glm::mat4_cast(glm::quat(tc.Rotation)) *
                glm::scale(glm::mat4(1.0f), tc.Scale);

            ImGuizmo::Manipulate(
                glm::value_ptr(view), glm::value_ptr(proj), m_GizmoType, ImGuizmo::LOCAL, glm::value_ptr(transform), nullptr, nullptr
            );
        ...
        ImGuizmo::ViewManipulate(glm::value_ptr(view), 8.0f, ImVec2(imageMax.x - 128.0f, imageMin.y), ImVec2(128.0f, 128.0f), 0x00000000);
    }
    ...
    auto EditorLayer::UI_Toolbar() -> void
    {
        ...
        for (size_t i = 0; i < buttons.size(); i++)
        {
            ...
            if (button.Icon)
            {
                ...
                drawList->AddImage(
                    ImGuiEx::CreateTextureRef(button.Icon->GetTexture()), { p0.x + pad, p0.y + pad }, { p1.x - pad, p1.y - pad },
    ...
    auto EditorLayer::UI_ViewportNotices() const -> void
    {
        ...
        const auto DrawNotice = [&](const char* notice)
        {
            ...
            drawList->AddRectFilled(
                { textPos.x - pad.x, textPos.y - pad.y }, { textPos.x + textSize.x + pad.x, textPos.y + textSize.y + pad.y },
