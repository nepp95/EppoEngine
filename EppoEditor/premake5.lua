project "EppoEditor"
    ...
    filter "configurations:Debug or Release"
        postbuildcommands {
            string.format('{COPYFILE} "%s/build/bin/%s/EppoScriptCore/EppoScriptCore.dll" "%s/build/bin/%s/EppoEditor"', EppoRoot, OutputDir, EppoRoot, OutputDir),
            string.format('{COPYFILE} "%s/build/bin/%s/EppoScriptCore/EppoScriptCore.pdb" "%s/build/bin/%s/EppoEditor"', EppoRoot, OutputDir, EppoRoot, OutputDir),
            string.format('{COPYFILE} "%s/build/bin/%s/EppoScriptCore/EppoScriptCore.deps.json" "%s/build/bin/%s/EppoEditor"', EppoRoot, OutputDir, EppoRoot, OutputDir),
            string.format('{COPYFILE} "%s/EppoEditor/runtimeconfig.json" "%s/build/bin/%s/EppoEditor"', EppoRoot, EppoRoot, OutputDir),
        }
    ...
    filter "configurations:Dist"
        postbuildcommands {
            string.format('{COPYFILE} "%s/build/bin/Release-%s-x86_64/EppoScriptCore/EppoScriptCore.dll" "%s/build/bin/%s/EppoEditor"',
                EppoRoot, os.host(), EppoRoot, OutputDir),
            string.format('{COPYFILE} "%s/build/bin/Release-%s-x86_64/EppoScriptCore/EppoScriptCore.deps.json" "%s/build/bin/%s/EppoEditor"',
                EppoRoot, os.host(), EppoRoot, OutputDir),
            string.format('{COPYFILE} "%s/EppoEditor/runtimeconfig.json" "%s/build/bin/%s/EppoEditor"', EppoRoot, EppoRoot, OutputDir),
        }
    ...
    filter { "system:windows", "configurations:Debug" }
        postbuildcommands {
            string.format('{COPYFILE} "%s" "%s/build/bin/%s/EppoEditor"', Dependencies.VulkanDxcompiler, EppoRoot, OutputDir),
            string.format('{COPYDIR} "%s" "%s/build/bin/%s/EppoEditor"', Dependencies.RuntimeDirectories.Debug, EppoRoot, OutputDir),
            string.format('{COPYDIR} "%s" "%s/build/bin/%s/EppoEditor"', Dependencies.RuntimeDirectories.DebugTracy, EppoRoot, OutputDir),
        }

    filter { "system:windows", "configurations:Release or Dist" }
        postbuildcommands {
            string.format('{COPYFILE} "%s" "%s/build/bin/%s/EppoEditor"', Dependencies.VulkanDxcompiler, EppoRoot, OutputDir),
            string.format('{COPYDIR} "%s" "%s/build/bin/%s/EppoEditor"', Dependencies.RuntimeDirectories.Release, EppoRoot, OutputDir),
        }
