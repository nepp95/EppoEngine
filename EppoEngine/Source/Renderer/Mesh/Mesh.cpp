    ...
    void Mesh::ProcessMaterials(const tinygltf::Model& model)
    {
        ...
        for (size_t i = 0; i < model.materials.size(); i++)
        {
            ...
            if (mat.pbrMetallicRoughness.metallicRoughnessTexture.index != -1)
                material->RoughnessMetallicMapIndex = model.textures[mat.pbrMetallicRoughness.metallicRoughnessTexture.
                                                                         index].source;
    ...
    MeshData Mesh::GetVertexData(const tinygltf::Model& model, const tinygltf::Mesh& mesh) const
    {
        ...
        for (const auto& primitive : mesh.primitives)
        {
            ...
            if (primitive.attributes.find("POSITION") != primitive.attributes.end())
            {
                ...
                positionData = reinterpret_cast<const float*>(&(model.buffers[bufferView.buffer].data[accessor.
            ...
            if (primitive.attributes.find("NORMAL") != primitive.attributes.end())
            {
                ...
                const auto& bufferView = model.bufferViews[accessor.bufferView];
                normalData = reinterpret_cast<const float*>(&(model.buffers[bufferView.buffer].data[accessor.byteOffset
            ...
            if (primitive.attributes.find("TEXCOORD_0") != primitive.attributes.end())
            {
                ...
                const auto& bufferView = model.bufferViews[accessor.bufferView];
                texCoordData = reinterpret_cast<const float*>(&(model.buffers[bufferView.buffer].data[accessor.
                    byteOffset + bufferView.byteOffset]));
            }
            ...
            {
                const auto* data = reinterpret_cast<const uint32_t*>(&buffer.data[bufferView.byteOffset + accessor.
                    byteOffset]);
            ...
            {
                const auto* data = reinterpret_cast<const uint16_t*>(&buffer.data[bufferView.byteOffset + accessor.
                    byteOffset]);
                for (size_t i = 0; i < accessor.count; i++)
                    meshData.Indices.push_back(data[i]);
            }
            ...
            {
                const auto* data = reinterpret_cast<const uint8_t*>(&buffer.data[bufferView.byteOffset + accessor.
                    byteOffset]);
                for (size_t i = 0; i < accessor.count; i++)
                    meshData.Indices.push_back(data[i]);
            }
