...
namespace Eppo
{
	...
	public:
		...
		[[nodiscard]] auto GetTime(const uint32_t frameIndex) const -> float { return frameIndex < m_Timestamps.size() ? m_Timestamps[frameIndex] : 0.0f; }
		...
		[[nodiscard]] auto GetTimeMs(const std::string_view name, const uint32_t frameIndex) const -> float { return GetTime(name, frameIndex) * 1000.0f; }
