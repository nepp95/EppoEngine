...
namespace Eppo
{
    ...
    void Scene::OnRenderRuntime(const Ref<SceneRenderer>& sceneRenderer)
    {
        ...
        for (const auto view = m_Registry.view<TransformComponent, CameraComponent>(); const auto e : view)
        {
    ...
    Ref<Scene> Scene::Copy(const Ref<Scene>& scene)
    {
        ...
        for (const auto idView = srcRegistry.view<IDComponent>(); const auto entity : idView)
        {
    ...
    Entity Scene::FindEntityByName(const std::string_view name)
    {
        ...
        for (const auto view = m_Registry.view<TagComponent>(); auto e : view)
        {
    ...
    void Scene::RenderScene(const Ref<SceneRenderer>& sceneRenderer)
    {
        ...
        for (const auto view = m_Registry.view<MeshComponent, TransformComponent>(); const EntityHandle entity : view)
        {
            if (auto [meshC, transform] = view.get<MeshComponent, TransformComponent>(entity); meshC.MeshHandle)
            {
        ...
        for (const auto view = m_Registry.view<PointLightComponent, TransformComponent>(); const EntityHandle entity : view)
        {
