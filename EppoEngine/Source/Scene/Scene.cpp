...
namespace Eppo
{
	...
	namespace
	{
		...
		// Translates entity's colliders into body-local shapes; sourceToBody must be shear-free (rotation under non-uniform scale is approximated).
		auto AppendColliders(Entity entity, const glm::mat4& sourceToBody, std::vector<ColliderData>& colliders) -> void
		{
			...
			const auto shapeOffset = [&](const glm::vec3& offset) { return glm::vec3(sourceToBody * glm::vec4(offset, 1.0f)); };

			if (entity.HasComponent<BoxColliderComponent>())
			{
				const auto& c = entity.GetComponent<BoxColliderComponent>();
				ColliderData data{ .Shape = ColliderShape::Box, .Offset = shapeOffset(c.Offset), .Rotation = rotation,
			...
			if (entity.HasComponent<SphereColliderComponent>())
			{
				...
				ColliderData data{ .Shape = ColliderShape::Sphere, .Offset = shapeOffset(c.Offset), .Rotation = rotation,
					.Density = c.Density, .Friction = c.Friction, .Restitution = c.Restitution };
			...
			if (entity.HasComponent<CapsuleColliderComponent>())
			{
				...
				ColliderData data{ .Shape = ColliderShape::Capsule, .Offset = shapeOffset(c.Offset), .Rotation = rotation,
					.Density = c.Density, .Friction = c.Friction, .Restitution = c.Restitution };
		...
		auto GatherColliders(Scene& scene, Entity source, const glm::mat4& invBodyPose, std::vector<ColliderData>& colliders,
			std::unordered_set<UUID>& visited) -> void
		{
		...
		auto TryGetMeshBounds(Entity entity, AABB& bounds) -> bool
		{
			...
			if (rawMeshHandle >= static_cast<uint64_t>(MeshPrimitiveType::Cone) && rawMeshHandle <= static_cast<uint64_t>(MeshPrimitiveType::Capsule))
			{
				bounds.Min = rawMeshHandle == static_cast<uint64_t>(MeshPrimitiveType::Capsule)
					? glm::vec3(-1.0f, -2.0f, -1.0f) : glm::vec3(-1.0f);
		...
		if (m_PhysicsWorld)
		{
			...
			const auto depthOf = [this](Entity entity)
			{
				...
				while (parentId && depth <= m_EntityMap.size())
				{
					...
					++depth;
					parentId = parent.HasComponent<RelationshipComponent>()
						? parent.GetComponent<RelationshipComponent>().Parent : UUID(0);
				}
				return depth;
			};
			...
			std::sort(bodies.begin(), bodies.end(), [](const auto& lhs, const auto& rhs) { return lhs.first < rhs.first; });
	...
	auto Scene::DuplicateEntity(Entity entity) -> Entity
	{
		...
		const Entity parent = entity.HasComponent<RelationshipComponent>()
			? GetEntityByUUID(entity.GetComponent<RelationshipComponent>().Parent) : Entity{};
		std::function<Entity(Entity, Entity)> duplicateHierarchy = [&](Entity source, Entity newParent) -> Entity
		{
			...
			const std::vector<UUID> children = hasRelationship ? source.GetComponent<RelationshipComponent>().Children : std::vector<UUID>{};
	...
	auto Scene::DestroyEntity(Entity entity) -> void
	{
		...
		if (const UUID parentId = entity.HasComponent<RelationshipComponent>()
			? entity.GetComponent<RelationshipComponent>().Parent : UUID(0))
		{
	...
	auto Scene::SetParent(Entity child, Entity parent) -> void
	{
		...
		if (const UUID oldParentId = child.HasComponent<RelationshipComponent>()
	...
	auto Scene::GetWorldTransform(Entity entity) -> glm::mat4
	{
		...
		for (size_t guard = 0; current; ++guard)
		{
			...
			const UUID parentId = current.HasComponent<RelationshipComponent>()
				? current.GetComponent<RelationshipComponent>().Parent : UUID(0);
			if (!parentId)
				break;
