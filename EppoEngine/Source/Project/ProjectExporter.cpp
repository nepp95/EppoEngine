...
namespace Eppo
{
    ...
    auto ProjectExporter::GetExportConfigurations(const ProjectExportOptions& options) const -> std::vector<ExportConfiguration>
	{
	    ...
	    {
	        const auto runtimeDirectory = options.DebugRuntimeDirectory.empty() ? sourceDirectory / "build" / "debug" / "EppoRuntime" : options.DebugRuntimeDirectory;
	    ...
	    {
	        const auto runtimeDirectory = options.ReleaseRuntimeDirectory.empty() ? sourceDirectory / "build" / "dist" / "EppoRuntime" : options.ReleaseRuntimeDirectory;
	        configurations.emplace_back(ExportConfiguration{
                .Name = "Release",
                #if defined(EP_PLATFORM_WINDOWS)
    ...
    ) const -> bool
	{
	    ...
	    exitCode = RunProcess("cmake", { "-E", "chdir", sourceDirectory.string(), "cmake", "--build", "--preset", configuration.CMakePreset, "--target", "EppoRuntime" });
