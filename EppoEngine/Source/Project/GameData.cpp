...
namespace Eppo
{
    auto GameData::Serialize(const std::filesystem::path& path) const -> bool
    {
        ...
        // Header
        if (!writer.WriteRaw<uint32_t>(PackFormat::Package.Magic) || !writer.WriteRaw<uint32_t>(PackFormat::Package.Version) ||
            !writer.WriteString(ProjectName) || !writer.WriteRaw<uint64_t>(static_cast<uint64_t>(StartScene)))
            return false;
        ...
        {
            if (!writer.WriteRaw<uint64_t>(static_cast<uint64_t>(handle)) ||
                !writer.WriteRaw<uint8_t>(static_cast<uint8_t>(packedAsset.Type)) || !writer.WriteBuffer(packedAsset.Payload))
                return false;
        }

        // Shaders
        if (!writer.WriteRaw<uint32_t>(PackFormat::Shader.Magic) || !writer.WriteRaw<uint32_t>(PackFormat::Shader.Version) ||
        ...
        {
            if (!writer.WriteString(shaderPath) || !writer.WriteString(source))
                return false;
        }
