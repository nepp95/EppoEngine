...
namespace Eppo
{
    namespace
    {
        ...
        VkDescriptorType ShaderResourceTypeToVkDescriptorType(const ShaderResourceType type)
        {
            ...
            if (type == ShaderResourceType::Sampler)
    ...
    std::unordered_map<ShaderStage, std::string> VulkanShader::PreProcess(std::string_view source) const
    {
        ...
        while (pos != std::string::npos)
        {
            ...
            shaderSources[Utils::StringToShaderStage(stage)] = (pos == std::string::npos)
    ...
    void VulkanShader::Compile(const ShaderStage stage, const std::string& source)
    {
        ...
        // Compile source
        const shaderc::SpvCompilationResult result = compiler.CompileGlslToSpv(source, ShaderStageToShaderCKind(stage),
                                                                               GetSpecification().Filepath.string().c_str(), options);
    ...
    void VulkanShader::CompileOrGetCache(const std::unordered_map<ShaderStage, std::string>& sources)
    {
        ...
        for (const auto& [stage, source] : sources)
        {
            ...
            if (Filesystem::Exists(cacheFile) && Filesystem::Exists(cacheHashFile))
            {
                ...
                // Check if cache needs to be busted
                if (std::string cacheHash = Filesystem::ReadText(cacheHashFile); cacheHash == hash)
                {
                    cacheVerified = true;
                }
            }
