...
namespace Eppo
{
    namespace
    {
        ...
        VkFrontFace CullFrontFaceToVkFrontFace(const CullFrontFace frontFace)
        {
            switch (frontFace)
            {
                case CullFrontFace::Clockwise:
                    return VK_FRONT_FACE_CLOCKWISE;
                case CullFrontFace::CounterClockwise:
        ...
        : m_Specification(std::move(specification))
    {
        ...
        {
            if (const auto format = colorAttachment.RenderImage->GetSpecification().Format; format != ImageFormat::Depth)
            {
        ...
        VK_CHECK(vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &graphicsPipelineCreateInfo, nullptr, &m_Pipeline),
        ...
        if (m_Specification.CreateDepthImage)
        {
            ...
            VulkanImage::TransitionImage(cmd, std::static_pointer_cast<VulkanImage>(image)->GetImageInfo().Image, VK_IMAGE_LAYOUT_UNDEFINED,
