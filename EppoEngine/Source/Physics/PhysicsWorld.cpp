...
namespace Eppo
{
    namespace Utils
    {
        ...
        static auto ToB3(const glm::vec3& v) -> b3Vec3
        {
            return b3Vec3{ v.x, v.y, v.z };
        }
        static auto FromB3(const b3Vec3& v) -> glm::vec3
        {
            return { v.x, v.y, v.z };
        }
        static auto ToB3(const glm::quat& q) -> b3Quat
        {
            return b3Quat{
                b3Vec3{ q.x, q.y, q.z },
                q.w
        ...
        static auto FromB3(const b3Quat& q) -> glm::quat
        {
    ...
    auto PhysicsWorld::CreateBody(
        const UUID entityId, const RigidBodyComponent& rigidBody, const glm::vec3& position, const glm::quat& rotation,
    ...
    auto PhysicsWorld::AttachCollider(const b3BodyId body, const ColliderData& collider) const -> void
    {
        ...
        switch (collider.Shape)
        {
            ...
            case ColliderShape::Capsule:
            {
                ...
                const b3Capsule capsule{ Utils::ToB3(collider.Offset - halfAxis), Utils::ToB3(collider.Offset + halfAxis),
                                         collider.Radius };
    ...
    auto PhysicsWorld::OverlapsSphere(const UUID entityId, const glm::vec3& center, const float radius) const -> bool
    {
        ...
        b3World_OverlapShape(
            m_WorldId, Utils::ToB3(center), &proxy, filter,
            [](const b3ShapeId shapeId, void* ctx) -> bool
            {
