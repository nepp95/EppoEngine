...
EppoEngine — a C++20 cross-platform (Windows/Linux) game engine + editor with C# scripting via CoreCLR (.NET 10) and Vulkan rendering through NVRHI. Built with CMake + vcpkg (manifest mode). CLAUDE.md holds the same core guidance for Claude Code; keep the two in sync when editing either.
...
Run a suite directly from `EppoEditor/` so source resources resolve correctly: `../build/debug/EppoEngineTesting/EppoEngineTesting Scripting`.
