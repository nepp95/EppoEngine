...
TEST(Physics, PhysicsWorld_DynamicBody_FallsUnderGravity)
{
    ...
    EXPECT_TRUE(world.GetPosition(id).y < startY - 0.1f);
}

TEST(Physics, PhysicsWorld_ApplyLinearImpulse_AddsVelocity)
{
    ...
    EXPECT_TRUE(world.GetLinearVelocity(id).x > 0.0f);
}

TEST(Physics, PhysicsWorld_StaticBody_DoesNotMove)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EXPECT_NEAR(5.0f, world.GetPosition(id).y, 0.001f);
}

TEST(Physics, PhysicsWorld_UnknownEntity_AccessorsAreSafe)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });

    const UUID missing;
    EXPECT_TRUE(!world.HasBody(missing));
    // Must not crash; velocity of an unknown body reads as zero.
    world.ApplyLinearImpulse(missing, { 1.0f, 0.0f, 0.0f });
    EXPECT_NEAR(0.0f, world.GetLinearVelocity(missing).x, 0.001f);
}

TEST(Physics, PhysicsWorld_SphereCollider_FallsUnderGravity)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
...
TEST(Physics, PhysicsWorld_CapsuleCollider_FallsUnderGravity)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
...
TEST(Physics, PhysicsWorld_Rotation_PreservedUnderGravity)
{
    ...
    EXPECT_NEAR(1.0f, dot, 0.001f);
}

TEST(Physics, PhysicsWorld_MultipleColliders_BodyFallsUnderGravity)
{
    ...
    EXPECT_TRUE(world.GetPosition(id).y < 5.0f - 0.1f);
    EXPECT_TRUE(world.GetLinearVelocity(id).y < -1.0f);
}

TEST(Physics, PhysicsWorld_KinematicBody_MovesWithSetVelocity)
{
    ...
    EXPECT_TRUE(world.GetPosition(id).x > startX + 0.5f);
    EXPECT_NEAR(2.0f, world.GetLinearVelocity(id).x, 0.01f);
}

TEST(Physics, PhysicsWorld_GravityScale_AffectsFallSpeed)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EXPECT_TRUE(lightY < 10.0f - 0.1f);
    EXPECT_TRUE(heavyY < lightY - 0.1f);
}

TEST(Physics, PhysicsWorld_LinearDamping_StopsBodyEventually)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f }); // gravity-free
    ...
    EXPECT_NEAR(0.0f, world.GetLinearVelocity(id).x, 0.01f);
    // The body should have moved but settled at a final position.
    EXPECT_TRUE(world.GetPosition(id).x > 0.0f);
}

TEST(Physics, PhysicsWorld_LockedLinearY_BodyDoesNotFallUnderGravity)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EXPECT_NEAR(10.0f, world.GetPosition(id).y, 1e-3f);
    ...
    EXPECT_TRUE(world.GetPosition(id).x > 0.1f);
    EXPECT_NEAR(10.0f, world.GetPosition(id).y, 1e-3f);
}

TEST(Physics, PhysicsWorld_LockedAngularAxes_BodyDoesNotTipOffLedge)
{
    ...
    EXPECT_TRUE(glm::abs(unlocked.w) < 0.99f);

    const glm::quat locked = restOnLedge(true);
    EXPECT_NEAR(1.0f, glm::abs(locked.w), 0.001f);
}

TEST(Physics, PhysicsWorld_CreateBody_UsesWorldPose)
{
    ...
    EXPECT_NEAR(1.0f, glm::abs(glm::dot(rotation, world.GetRotation(id))), 1e-5f);
}

TEST(Physics, PhysicsWorld_GetShapeCount_CountsAttachedColliders)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_EQ(2, world.GetShapeCount(id));
    EXPECT_EQ(0, world.GetShapeCount(UUID()));
}

TEST(Physics, PhysicsWorld_RotatedBoxCollider_CollidesAtRotatedPose)
{
    ...
    EXPECT_TRUE(world.GetPosition(ballId).y > 0.3f);
}

TEST(Physics, PhysicsWorld_RotatedCapsuleCollider_CollidesAtRotatedPose)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_TRUE(world.GetPosition(ballId).y > 0.5f);
}

TEST(Physics, PhysicsWorld_RotatedSphereCollider_KeepsOffsetCenter)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_TRUE(world.GetPosition(ballId).x > 1.5f);
}

TEST(Physics, PhysicsWorld_ZeroHeightCapsule_CollidesAsSphere)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_TRUE(world.GetPosition(ballId).x > 0.5f);
}

TEST(Physics, PhysicsWorld_CylinderCollider_BlocksSphere)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_NEAR(1.5f, world.GetPosition(topBallId).y, 0.1f);
}

TEST(Physics, PhysicsWorld_DynamicCylinder_RestsOnFloorAtFullHeight)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EXPECT_NEAR(1.5f, world.GetPosition(cylinderId).y, 0.1f);
}

TEST(Physics, PhysicsWorld_ZeroExtentBoxCollider_StillBlocks)
{
    PhysicsWorld world({ 0.0f, 0.0f, 0.0f });
    ...
    EXPECT_TRUE(world.GetPosition(ballId).y > 0.0f);
}

TEST(Physics, PhysicsWorld_CastRay_HitsStaticBoxBelow)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EP_REQUIRE(hit.Hit);
    EXPECT_NEAR(0.5f, hit.Point.y, 0.05f);
    EXPECT_NEAR(1.0f, hit.Normal.y, 0.05f);
    EXPECT_NEAR(4.5f, hit.Distance, 0.1f);
    EXPECT_EQ(static_cast<uint64_t>(floorId), static_cast<uint64_t>(hit.EntityId));
}

TEST(Physics, PhysicsWorld_CastRay_MissesWhenOutOfRange)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
...
TEST(Physics, PhysicsWorld_CastRay_IgnoresInitialOverlap)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
...
TEST(Physics, PhysicsWorld_OverlapsSphere_TrueInsideRadius_FalseOutside)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    EXPECT_TRUE(world.OverlapsSphere(id, { 0.0f, 0.0f, 0.0f }, 2.0f));
    EXPECT_TRUE(!world.OverlapsSphere(id, { 10.0f, 0.0f, 0.0f }, 2.0f));
}

TEST(Physics, PhysicsWorld_OverlapsSphere_MatchesOnlyTargetBody)
{
    PhysicsWorld world({ 0.0f, -9.81f, 0.0f });
    ...
    const UUID missing;
    EXPECT_TRUE(world.OverlapsSphere(a, { 0.0f, 0.0f, 0.0f }, 2.0f));
    EXPECT_TRUE(world.OverlapsSphere(b, { 0.0f, 0.0f, 0.0f }, 2.0f));
    EXPECT_TRUE(!world.OverlapsSphere(missing, { 0.0f, 0.0f, 0.0f }, 2.0f));
    EXPECT_TRUE(!world.OverlapsSphere(a, { 10.0f, 0.0f, 0.0f }, 2.0f));
}
...
TEST(Physics, Scene_RuntimeBodyWithoutCollider_StillFalls)
{
    ...
    EXPECT_EQ("NoCollider", scene->GetColliderlessRigidBodies().front());
    StepScene(scene, 30);
    const float y = entity.GetComponent<TransformComponent>().Translation.y;
    scene->OnRuntimeStop();
...
TEST(Physics, Scene_FitSphereColliderToMesh_UsesPrimitiveBounds)
{
    ...
    CHECK_VEC3_CLOSE(glm::vec3(0.0f), collider.Offset, 0.001f);
    EXPECT_NEAR(1.0f, collider.Radius, 0.001f);
}
...
TEST(Physics, Scene_FitCapsuleColliderToMesh_UsesPrimitiveBounds)
{
    ...
    CHECK_VEC3_CLOSE(glm::vec3(0.0f), collider.Offset, 0.001f);
    EXPECT_NEAR(1.0f, collider.Radius, 0.001f);
    EXPECT_NEAR(2.0f, collider.Height, 0.001f);
}
...
TEST(Physics, Scene_RuntimeMultipleChildBoxColliders_EachContributesShape)
{
    ...
    EXPECT_EQ(3, scene->GetPhysicsWorld()->GetShapeCount(root.GetUUID()));
    scene->OnRuntimeStop();
}
...
TEST(Physics, Scene_RuntimeNestedRigidBody_StartsOwnBodyAndBoundsGathering)
{
    ...
    EXPECT_EQ(1, physics->GetShapeCount(root.GetUUID()));
    EXPECT_EQ(2, physics->GetShapeCount(child.GetUUID()));
...
TEST(Physics, Scene_CopiedThenRun_GathersChildCollidersOntoRootBody)
{
...
TEST(Physics, Scene_RuntimeParentedDynamicRoot_SyncsWithoutDoubleParentTransform)
{
    ...
    EXPECT_NEAR(10.0f, worldPosition.x, 0.01f);
    EXPECT_TRUE(worldPosition.y < 6.0f - 0.1f);
    EXPECT_NEAR(0.0f, worldPosition.z, 0.01f);
...
TEST(Physics, Scene_RuntimeColliderWithoutRigidBodyAncestor_GetsNoBody)
{
    ...
    scene->OnRuntimeStart();
    EXPECT_TRUE(!scene->GetPhysicsWorld()->HasBody(orphan.GetUUID()));

    StepScene(scene, 10);
    // No body means physics never writes this transform.
    EXPECT_NEAR(5.0f, orphan.GetComponent<TransformComponent>().Translation.y, 1e-5f);
    scene->OnRuntimeStop();
}
...
TEST(Physics, SceneSerializer_PhysicsComponents_SurviveSaveAndLoad)
{
    ...
    EP_REQUIRE(static_cast<bool>(entity));

    const auto& rb = entity.GetComponent<RigidBodyComponent>();
    EXPECT_TRUE(rb.Type == RigidBodyComponent::BodyType::Dynamic);
    EXPECT_NEAR(2.0f, rb.GravityScale, 1e-5f);
    EXPECT_NEAR(0.3f, rb.LinearDamping, 1e-5f);
    EXPECT_NEAR(0.4f, rb.AngularDamping, 1e-5f);
    EXPECT_TRUE(!rb.LockLinearX);
    EXPECT_TRUE(rb.LockLinearY);
    EXPECT_TRUE(!rb.LockLinearZ);
    EXPECT_TRUE(rb.LockAngularX);
    EXPECT_TRUE(!rb.LockAngularY);
    EXPECT_TRUE(rb.LockAngularZ);
    ...
    EXPECT_NEAR(1.5f, box.Density, 1e-5f);
    EXPECT_NEAR(0.25f, box.Friction, 1e-5f);
    EXPECT_NEAR(0.6f, box.Restitution, 1e-5f);

    const auto& sphere = entity.GetComponent<SphereColliderComponent>();
    EXPECT_NEAR(0.75f, sphere.Radius, 1e-5f);
    CHECK_VEC3_CLOSE(glm::vec3(0.4f, 0.5f, 0.6f), sphere.Offset, 1e-5f);
    EXPECT_NEAR(2.5f, sphere.Density, 1e-5f);
    EXPECT_NEAR(0.35f, sphere.Friction, 1e-5f);
    EXPECT_NEAR(0.7f, sphere.Restitution, 1e-5f);

    const auto& capsule = entity.GetComponent<CapsuleColliderComponent>();
    EXPECT_NEAR(0.9f, capsule.Radius, 1e-5f);
    EXPECT_NEAR(1.8f, capsule.Height, 1e-5f);
    CHECK_VEC3_CLOSE(glm::vec3(0.7f, 0.8f, 0.9f), capsule.Offset, 1e-5f);
    EXPECT_NEAR(3.5f, capsule.Density, 1e-5f);
    EXPECT_NEAR(0.45f, capsule.Friction, 1e-5f);
    EXPECT_NEAR(0.8f, capsule.Restitution, 1e-5f);

    const auto& cylinder = entity.GetComponent<CylinderColliderComponent>();
    EXPECT_NEAR(1.1f, cylinder.Radius, 1e-5f);
    EXPECT_NEAR(2.2f, cylinder.Height, 1e-5f);
    CHECK_VEC3_CLOSE(glm::vec3(1.0f, 1.1f, 1.2f), cylinder.Offset, 1e-5f);
    EXPECT_NEAR(4.5f, cylinder.Density, 1e-5f);
    EXPECT_NEAR(0.55f, cylinder.Friction, 1e-5f);
